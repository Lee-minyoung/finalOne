<template>
  ...
  :class="{ 'table-active': selectedOrdNo === item.spm_no }">
    ...
    <div class="text-end mb-3">
      <button class="btn btn-primary">출하완료 처리</button>
    </div>
    ...
    <div class="info-box">
      ...
      <div class="product-row">
       제품번호<input class="info-input" placeholder="제품ID" v-model="productInfo.prd_no" readonly />
  제품명<input class="info-input" placeholder="제품명" v-model="productInfo.prd_nm" readonly />
  수량<input class="info-input" placeholder="수량" v-model="productInfo.qty" readonly />
      </div>
    </div>
  </div>
  </div>
...
export default {
  ...
  async fetchDetail(item) {
   try {
    // 납품처 정보
    const res = await axios.get('/api/spmMrk/detail', { params: { ord_no: item.spm_no } });
    if (res.data && res.data.length > 0) {
      this.detailInfo = res.data[0];
    }
    // 제품상세정보
    const res2 = await axios.get('/api/spmMrk/detail/product', { params: { spm_no: item.spm_no } });
    if (res2.data && res2.data.length > 0) {
      // 여러 제품이 있을 수 있으니 첫 번째만 입력
      this.productInfo = {
        prd_no: res2.data[0].prd_no,
        prd_nm: res2.data[0].prd_nm,
        qty: res2.data[0].qty
      };
    } else {
      this.productInfo = { prd_no: '', prd_nm: '', qty: '' };
    }
  } catch (err) {
    alert('상세정보 불러오기 실패');
  }
